...
**Status:** 2026-06-20 · **Measured baseline:** `334/599` (55.8%) on current code (main + P6b.2 #290 + P1 #291) — P1 added **net +11, zero regressions** (`language 4→15`). Prior measured point `323/599` (main + P6b.2) against `hl7.fhir.uv.tx-ecosystem#current` (tests v1.90, runner v6.9.10). Up from 318 after P7 `abstract` (#287), P6a supplement-missing 404 (#288), P6b.1 used-supplement echo (#289), P6b.2 lookup source (#290). **Refreshed gap pools (fails):** version 62, permutations 33, validation 31, language 22 + language2 13 (=35, **P1 in progress**), overload 19, parameters 16, extensions 11, default-valueset-version 8, notSelectable 6, deprecated 5. **P1 strategy = (a) + resource-language-as-displayLanguage** (chosen 2026-06-20). **P1 partial DONE:** inline `$expand` now resolves each member's display by the effective language (request → VS expansion-parameter extension → CS resource language), keeps alternate-language designations (de-duped, no spurious `use`/`language`), and echoes the VS-extension `displayLanguage`; validate-code's display-severity now matches designations only in the requested language (avoids the `bad-language` coupling regression). **language 4→15** (P1 core 4→11; + de-CH exact-language display pick 11→13; + VS.language echo & root `ValueSet.language` 13→15), validation neutral. Remaining language fails (11): **Accept-Language header** ×4 (`*-header` — the op cannot see the HTTP header; needs new plumbing, and per decision 2026-06-20 the header ranks BELOW the resource/VS-declared language); `xform` ×6 (`preferredForLanguage` use on the primary display once it is demoted to a designation by a non-resource displayLanguage); `en-designations` ×1 (designation-filter count). NB de-CH is a BCP-47 region subtag (`de`=German + `-CH`=Switzerland); the pick now prefers an exact language match over a region match so the `de` primary display wins over the `de-CH` designation.
