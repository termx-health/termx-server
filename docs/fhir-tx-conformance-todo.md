...
- **P7 (enum-nested, ~4)** — confirmed: `value_set_expand(text)` resolves an enumerated `compose.include.concept` against **top-level** `CodeSystem.concept` only; `code2a`/`code2b` (nested `concept.concept` in the CS) are dropped (`Expected 5 Actual 3`). The DB import DOES create them (the whole-CS `all-hierarchy` expand finds them), so it's the **enumerated-include resolution** in the SQL. **Decision needed:** (a) Java post-hoc — after `expandFromJson`, add any enumerated codes missing from the result by looking them up in the tx-resource CS (needs sourcing display/designations/inactive from tx-resource, ~30 lines, inline-only); or (b) fix `value_set_expand` SQL to flatten nested concepts (covers stored path, DB change). I lean (a).
