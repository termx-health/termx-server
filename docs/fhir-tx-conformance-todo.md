...
- **P9a (errors 400-cluster: unknown-system1/2, combination-ok/bad — 4 in-app tests) — TRUE ROOT CAUSE FOUND via wire capture; it is NOT an unknown-system bug.** Captured the validator's real request (logging proxy in front of `/fhir`): the `txTests` runner bundles a suite's **entire setup as `tx-resource` params on every request**. The `errors` suite's setup includes the **deliberately-malformed** `broken-filter`/`broken-filter2` ValueSets — a `compose.include.filter` with **no `value`** (`value` is `1..1` in R5). kefhir's `ResourceProfileValidator` (`com.kodality.kefhir.validation`, an `OperationInterceptor`) runs HAPI profile validation over the **whole** input `Parameters`, *including the bundled supporting resources*, and throws **400** with `ValueSet.compose.include.filter.value: minimum required = 1, but only found 0` at location `Parameters.parameter[N].resource/*ValueSet/broken-filter*/…`. So `unknown-system1/2` and `combination-ok/bad` are rejected purely because an **unrelated supporting resource** is malformed — the operation never runs. (The `broken-filter-*` tests themselves are a *separate* issue: in-app they reach 200 and fail on issue-text, because their operation target is the broken VS resolved from the DB, not a bundled tx-resource.) The reference engine validates the operation envelope but **tolerates malformed supporting resources** — it's the operation's job to handle them, not the input validator's. **This is a kefhir-layer fix, and kefhir is consumed as a published GitHub Packages jar (`kefhirVersion=R5.10`), not a composite build — so it needs your steer:** (a) **fix kefhir** `ResourceProfileValidator` so the *operation-input* path filters out validation errors located inside a `parameter[].resource` (supporting resource), keeping envelope errors — correct + minimal, but requires publishing `kefhir R5.11` to GitHub Packages (outward-facing release) + bumping `gradle.properties`; (b) **termx-server-local override** — a `@Replaces(ResourceProfileValidator.class)` bean that skips/loosens operation-input profile validation (no kefhir release, but duplicates kefhir logic and Micronaut field-injection into the inherited private deps is fragile); (c) skip — only ~4 tests. I lean (a): it's the correct layer and the same relaxation any tx server needs, but I won't publish a package release without your go-ahead. The earlier "graceful unknown-system degradation" idea is **moot** — the request never reaches that code.
