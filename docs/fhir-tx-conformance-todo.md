...
**Status:** 2026-06-20 · **Measured baseline:** `348/599` (58.1%) on current code (main + P6b.2 #290 + P1 #291 + P8 #292). Session arc: `323` (P6b.2) → `+15` P1 display-language (#291, `language 4→19`) → `338` → `+10` P8 VS-import (#292, `default-valueset-version` indirect-* + `permutations` good-*-import/P3) → `348`. Zero regressions except one deferred edge (`indirect-validation-zero-pinned`). against `hl7.fhir.uv.tx-ecosystem#current` (tests v1.90, runner v6.9.10). Up from 318 after P7 `abstract` (#287), P6a supplement-missing 404 (#288), P6b.1 used-supplement echo (#289), P6b.2 lookup source (#290). **Refreshed gap pools (fails):** version 62, permutations 33, validation 31, language 22 + language2 13 (=35, **P1 in progress**), overload 19, parameters 16, extensions 11, default-valueset-version 8, notSelectable 6, deprecated 5. **P1 strategy = (a) + resource-language-as-displayLanguage** (chosen 2026-06-20). **P1 partial DONE:** inline `$expand` now resolves each member's display by the effective language (request → VS expansion-parameter extension → CS resource language), keeps alternate-language designations (de-duped, no spurious `use`/`language`), and echoes the VS-extension `displayLanguage`; validate-code's display-severity now matches designations only in the requested language (avoids the `bad-language` coupling regression). **language 4→19** (P1 core 4→11; + de-CH exact-language display pick →13; + VS.language echo & root `ValueSet.language` →15; + **Accept-Language header →19**), validation neutral, **net +15**. Accept-Language is read via Micronaut `ServerRequestContext` and ranks BELOW the request param / VS-declared language / CS resource language (decision 2026-06-20); only the `*-header` requests carry it (no blast radius). NB de-CH is a BCP-47 region subtag (`de`=German + `-CH`=Switzerland); the pick prefers an exact language match over a region match so the `de` primary display wins over the `de-CH` designation. **Remaining language fails (7):** `xform` ×6 — a transform/translate path where, with a non-resource displayLanguage, the member display must be the **display-typed** designation in that language (e.g. de `Anzeige 2`, NOT a de synonym `Mein erster Code`), the demoted primary display carries a `preferredForLanguage` use, and the case bundles extra tx-resources I couldn't reproduce by hand (manual inline request trips profile validation). Tried `preferredForLanguage` use + a display-type pick tiebreaker — advanced the diff (use OK) but `contains[1].display` still resolves a synonym; reverted (unverified, flips nothing) for a focused follow-up. `en-designations` ×1 — designation-filter count.
...
### P2 — precise findings (investigated 2026-06-20, post-P1; partial attempt reverted)
With P1 merged, the display half of `validation` is ~7 fails: 4× `display` param (`good/bad-language-none` × code/coding) + 3× `error` severity (`bad-language-vslang/vs/header`). Three distinct issues, in order of clarity:
1. **The internal expand drops the supporting CodeSystems.** `validateInline` builds a fresh `expandReq` with only `valueSet` + `includeDesignations` (+ `displayLanguage`) — it does NOT forward the request's `tx-resource` CodeSystems, so P1's display resolution (resource language, primary display) gets no metadata. **Fix: forward the `tx-resource` params to `expandReq`** (one-liner; tried it, it changes the display but exposes #2).
2. **The response `display` param must be the concept's PRIMARY display, not the displayLanguage-selected one.** e.g. validate code2aII with `displayLanguage=de` → expected response `display` = `"Display 2aII"` (the en primary), while the issue MESSAGE ("valid display is …") wants the de display. termx uses `match.getDisplay()` (the displayLanguage-selected display from the inline expand) for BOTH. Need to separate: `display` param = primary display; message = localized. Either expand once without displayLanguage for the param + read designations for the message, or carry the primary display through.
3. **`bad-language` severity must use the EFFECTIVE display language for the `*-vslang/-vs/-header` variants.** P1's validate-code `matchesAny` language filter (committed in #291) only consults the request `displayLanguage` PARAM; for vslang/vs/header the language comes from the VS/extension/Accept-Language, so the filter sees null → matches any → wrong (information) severity. Mirror P1's effective-language resolution here.
Also shares the xform display-pick refinement (prefer a display-typed designation + exact-language over a synonym/region). NB the inline expand resolves concepts from the **DB**, where the tx-ecosystem's reused `en-multi` canonical may carry extra designations (#248 collision) — verify the picked display against the tx-resource CS, not just the DB.
**Status:** reverted the tx-resource-forwarding spike (correct in principle but flips nothing alone and risks validate-code regression unverified). A focused P2 PR should do #1+#2+#3 together + one full-run verification.

### ⛔ P2 + xform ROOT BLOCKER — loadSetup designation pollution (found 2026-06-20)
The remaining P2 display cases AND the 6 `language/xform` cases are **blocked by DB designation pollution**, not display logic. Verified by DB query: `en-multi` `code2` carries a spurious `de` designation `"Mein erster Code"` (code1's German) on top of its correct `de-CH "Anzeige 2"` / `es` / `en` ones. So `displayLanguage=de` resolves the polluted `"Mein erster Code"` instead of `"Anzeige 2"`. Cause: the tx-ecosystem reuses the same canonical url (`…/en-multi`) across suites with DIFFERENT concept content; `loadSetup` + canonical reconciliation (#248 family) **merge their designations onto one entity** in the DB, and the inline `$expand`/`$validate-code` resolve designations from that polluted DB (not from the bundled tx-resource CS). No display-pick refinement can fix a polluted candidate set. **Two possible real fixes (both larger, separate):** (a) import/loader — don't merge designations across colliding canonicals (or version-isolate them); (b) inline expand — resolve member designations from the request's tx-resource CodeSystem when supplied, instead of the DB. Until then, xform (6) + the bulk of P2's display cases can't pass. The committed P1 display logic (display-by-effective-language, exact/region/display-type pick) is correct and verified; it's the data that's wrong here.
