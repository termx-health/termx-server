...
- **P9a (errors unknown-system, ~4)** — `$validate-code` with `system=<unknown CS>` (e.g. `simpleX`) currently **errors (4xx)** instead of the graceful **200**: expected `[code, issues(error/not-found/not-found), message "A definition for CodeSystem '<sys>' could not be found, so the code cannot be validated", result=false, system, x-caused-by-unknown-system=<sys>]` — i.e. the **unknown-SYSTEM** analog of the `unknownSystemVersion` degradation I already built. Two parts: trace the 4xx (the inline expand of a VS whose include system is entirely unresolvable throws) and add the unknown-system graceful shape. `combination-ok/bad` also 4xx (likely a multi-include/`x-caused-by` combination). `broken-filter*` is a separate issue-text match. **Decision:** go with my proposal (graceful unknown-system, parallel to unknown-version).
