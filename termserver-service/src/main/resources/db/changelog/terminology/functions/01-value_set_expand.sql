...
with vs_rule_set as (
    ...
    where vsv.sys_status = 'A' and vs.id = p_value_set and (p_value_set_version is null or vsv.version = p_value_set_version) and
        (p_value_set_version is not null or tsrange(vsv.release_date, vsv.expiration_date) @> now()::timestamp)
    limit 1
),
    rule_set as (
                     ...
                     where ((ir ->> 'codeSystem') is not null and ((ir ->> 'codeSystemVersion') is not null or (ir ->> 'lockedDate') is not null)
                             and (exists(select 1
                                         ...
                                         on csev.id = evcsvm.code_system_entity_version_id and csev.sys_status = 'A'
                                                  inner join terminology.code_system_entity cse
                                         on cse.id = csev.code_system_entity_id and cse.sys_status = 'A'
                                 ...
                                 and (not exists(select jsonb_array_elements(ir -> 'filters'))
                                     or exists(select jsonb_array_elements(ir -> 'filters') where 1 = 1)))) or
                         ((ir ->> 'valueSet') is not null and (ir ->> 'valueSetVersion') is not null
                                 and exists(select 1
                       ...
                       where ((er ->> 'codeSystem') is not null and ((er ->> 'codeSystemVersion') is not null or (er ->> 'lockedDate') is not null)
                               and (exists(select 1
                                   ...
                                   and (not exists(select jsonb_array_elements(er -> 'filters'))
                                       or exists(select jsonb_array_elements(er -> 'filters') where 1 = 1)))) or
                           ((er ->> 'valueSet') is not null and (er ->> 'valueSetVersion') is not null
                                   and exists(select 1
