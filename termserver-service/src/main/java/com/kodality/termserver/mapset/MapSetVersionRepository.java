...
public class MapSetVersionRepository extends BaseRepository {
  ...
  public void save(MapSetVersion version) {
    ...
    SqlBuilder sb = ssb.buildSave("map_set_version", "id");
    Long id = jdbcTemplate.queryForObject(sb.getSql(), Long.class, sb.getParams());
    version.setId(id);
  }

  public MapSetVersion getVersion(String mapSet, String version) {
    String sql = "select * from map_set_version where sys_status = 'A' and map_set = ? and version = ?";
    return getBean(sql, bp, mapSet, version);
  }

  public List<MapSetVersion> getVersions(String mapSet) {
    String sql = "select * from map_set_version where sys_status = 'A' and map_set = ?";
    return getBeans(sql, bp, mapSet);
  }

  public QueryResult<MapSetVersion> query(MapSetVersionQueryParams params) {
    return query(params, p -> {
      SqlBuilder sb = new SqlBuilder("select count(1) from map_set_version where sys_status = 'A'");
      sb.append(filter(params));
      return queryForObject(sb.getSql(), Integer.class, sb.getParams());
    }, p -> {
      SqlBuilder sb = new SqlBuilder("select * from map_set_version where sys_status = 'A'");
      sb.append(filter(params));
  ...
  public void activate(String mapSet, String version) {
    String sql = "update map_set_version set status = ? where map_set = ? and version = ? and sys_status = 'A' and status <> ?";
  ...
  public void upsertEntityVersions(List<MapSetEntityVersion> entityVersions, Long mapSetVersionId) {
    ...
    entityVersions.forEach(v -> {
      ...
      SqlBuilder sb = ssb.buildUpsert("entity_version_map_set_version_membership", "map_set_entity_version_id", "map_set_version_id");
      jdbcTemplate.update(sb.getSql(), sb.getParams());
    });
  }
