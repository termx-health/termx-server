...
public class TagRepository extends BaseRepository {
  ...
  public void save(Tag tag) {
    ...
    SqlBuilder sb = ssb.buildSave("thesaurus.tag", "id");
    Long id = jdbcTemplate.queryForObject(sb.getSql(), Long.class, sb.getParams());
    tag.setId(id);
  }

  public List<Tag> loadAll() {
    String sql = "select * from thesaurus.tag where sys_status = 'A'";
    return getBeans(sql, bp);
  }

  public Tag load(Long id) {
    String sql = "select * from thesaurus.tag where id = ? and sys_status = 'A'";
