...
public class UcumValueSetExpandProvider extends ValueSetExternalExpandProvider {
  ...
  public UcumValueSetExpandProvider(UcumConceptResolver ucumConceptResolver, UcumSupplementDesignationService ucumSupplementDesignationService) {
    this.ucumConceptResolver = ucumConceptResolver;
    this.ucumSupplementDesignationService = ucumSupplementDesignationService;
  }
  ...
  public List<ValueSetVersionConcept> ruleExpand(ValueSetVersionRule rule, ValueSetVersion version, String preferredLanguage) {
    ...
    if (CollectionUtils.isEmpty(rule.getFilters())) {
      ...
      ucumSupplementDesignationService.enrich(ruleConcepts, preferredLanguage);
      return ruleConcepts;
    }
