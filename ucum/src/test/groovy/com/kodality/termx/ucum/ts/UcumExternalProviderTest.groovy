...
class UcumExternalProviderTest extends Specification {
  ...
  private UcumValueSetExpandProvider expandProvider() {
    new UcumValueSetExpandProvider(new UcumConceptResolver(new UcumMapper(), ucumService), emptySupplementService())
  }

  private static UcumSupplementDesignationService emptySupplementService() {
    new UcumSupplementDesignationService(
        new CodeSystemService(null, null, null, null, null) {
          @Override
          QueryResult<CodeSystem> query(CodeSystemQueryParams params) {
            return QueryResult.empty()
          }
        },
        new ConceptService(null, null, null, null, null, List.of()) {
          @Override
