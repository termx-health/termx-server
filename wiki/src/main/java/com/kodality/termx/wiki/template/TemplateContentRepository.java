...
public class TemplateContentRepository extends BaseRepository {
  ...
  public void save(TemplateContent content, Long templateId) {
    ...
    SqlBuilder sb = ssb.buildSave("wiki.template_content", "id");
    Long id = jdbcTemplate.queryForObject(sb.getSql(), Long.class, sb.getParams());
    content.setId(id);
  }

  public TemplateContent load(Long id) {
    String sql = "select * from wiki.template_content where sys_status = 'A' and id = ?";
    return getBean(sql, bp, id);
  }

  public List<TemplateContent> loadAll(Long templateId) {
    String sql = "select * from wiki.template_content where sys_status = 'A' and template_id = ?";
    return getBeans(sql, bp, templateId);
  }

  public void retain(List<TemplateContent> contents, Long templateId) {
    SqlBuilder sb = new SqlBuilder("update wiki.template_content set sys_status = 'C'");
  ...
  public List<String> findContent(Long templateId, String lang) {
    String sql = "select tc.content from wiki.template_content tc where tc.sys_status = 'A' and tc.template_id = ? and tc.lang = ?";
