...
DB_NAME="${1:-termx}"
DB_OWNER="${DB_OWNER:-tx_admin}"
APP_USER="${APP_USER:-tx_app}"
VIEWER_USER="${VIEWER_USER:-tx_viewer}"
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '${VIEWER_USER}') THEN
    EXECUTE 'CREATE ROLE ${VIEWER_USER} NOLOGIN NOSUPERUSER INHERIT NOCREATEDB NOCREATEROLE NOREPLICATION';
  END IF;
END
\$\$;
