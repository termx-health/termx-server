package org.termx.terminology.fileimporter.valueset;
...
import org.termx.terminology.fileimporter.valueset.utils.ValueSetFileImportProcessor;
import org.termx.terminology.fileimporter.valueset.utils.ValueSetFileImportRequest;
import org.termx.terminology.fileimporter.valueset.utils.ValueSetFileImportRequest.FileProcessingValueSet;
import org.termx.terminology.fileimporter.valueset.utils.ValueSetFileImportRequest.FileProcessingValueSetVersion;
...
import static org.termx.terminology.fileimporter.valueset.utils.ValueSetFileImportMapper.toValueSet;
...
public class ValueSetFileImportService {
  ...
  private void prepare(ValueSetFileImportRequest request) {
    if (request.getVersion().getRule() != null && request.getVersion().getRule().getCodeSystemUri() != null) {
      CodeSystemVersion csv = request.getVersion().getRule().getCodeSystemVersion() != null ?
          codeSystemVersionService.loadVersionByUri(request.getVersion().getRule().getCodeSystemUri(), request.getVersion().getRule().getCodeSystemVersion()).get() :
          codeSystemVersionService.loadLastVersionByUri(request.getVersion().getRule().getCodeSystemUri());
      if (csv == null) {
