...
public class SpaceRepository extends BaseRepository {
  ...
  public void save(Space space) {
    ...
    SqlBuilder sb = ssb.buildSave("terminology.space", "id");
    Long id = jdbcTemplate.queryForObject(sb.getSql(), Long.class, sb.getParams());
    space.setId(id);
  }

  public Space load(Long id) {
    String sql = "select * from terminology.space where id = ? and sys_status = 'A'";
    return getBean(sql, bp, id);
  }

  public Space load(String code) {
    String sql = "select * from terminology.space where code = ? and sys_status = 'A'";
    return getBean(sql, bp, code);
  }

  public QueryResult<Space> query(SpaceQueryParams params) {
    return query(params, p -> {
      SqlBuilder sb = new SqlBuilder("select count(1) from terminology.space s where s.sys_status = 'A'");
      sb.append(filter(params));
      return queryForObject(sb.getSql(), Integer.class, sb.getParams());
    }, p -> {
      SqlBuilder sb = new SqlBuilder("select * from terminology.space s where s.sys_status = 'A'");
      sb.append(filter(params));
