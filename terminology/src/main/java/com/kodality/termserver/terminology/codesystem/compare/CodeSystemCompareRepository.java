...
import com.kodality.termserver.terminology.codesystem.compare.CodeSystemCompareResult.CodeSystemCompareResultChange;
import com.kodality.termserver.terminology.codesystem.compare.CodeSystemCompareResult.CodeSystemCompareResultDiff;
...
public class CodeSystemCompareRepository extends BaseRepository {
  public CodeSystemCompareResult compare(Long sourceCsVersionId, Long targetCsVersionId) {
    String s = """
        select c.code, csev.status, csev.id version_id
    ...
    String sql = "with" +
        ...
        "   'old', jsonb_build_object('status', c1.status, 'properties', (" + p + " = c1.version_id))" +
        " , 'mew', jsonb_build_object('status', c2.status, 'properties', (" + p + " = c2.version_id))" +
        " )) diff" +
