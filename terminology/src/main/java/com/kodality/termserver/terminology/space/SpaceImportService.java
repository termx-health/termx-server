...
import com.kodality.termserver.ts.space.spacepackage.PackageStatus;
...
import com.kodality.termserver.terminology.space.spacepackage.version.PackageVersionService;
import com.kodality.termserver.ts.space.server.TerminologyServer;
import com.kodality.termserver.ts.space.server.TerminologyServerQueryParams;
...
public class SpaceImportService {
  ...
  @Transactional
  public void importSpace(Publisher<CompletedFileUpload> file) {
    if (file == null) {
      return;
    }
    String yaml = new String(readBytes(Flowable.fromPublisher(file).firstOrError().blockingGet()));
    SpaceOverview overview = toJson(yaml);
  ...
  private SpaceOverview toJson(String yaml) {
    try {
