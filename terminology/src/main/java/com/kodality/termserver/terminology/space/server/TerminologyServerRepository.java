package com.kodality.termserver.terminology.space.server;
...
public class TerminologyServerRepository extends BaseRepository {
  ...
  private SqlBuilder filter(TerminologyServerQueryParams params) {
    ...
    sb.appendIfNotNull("and exists(select 1 from terminology.space s where s.id = ? and ts.code in (select jsonb_array_elements_text(s.terminology_servers)) and s.sys_status = 'A')", params.getSpaceId());
    return sb;
  }
