...
public class C4RuleValidator implements CodeSystemRuleValidator {
  ...
  public List<ChecklistAssertionError> validate(CodeSystem codeSystem, List<Concept> concepts, List<ChecklistWhitelist> whitelists) {
    return concepts.stream().flatMap(c -> c.getVersions().stream())
        ...
        .map(p -> new ChecklistAssertionError().setError(String.format("The concept's '%s' designation length exceeds the maximum allowed number of symbols (%d).", p.getKey(), MAX_DESIGNATION_LENGTH))).toList();
  }
}
