        ...
        .flatMap(v -> {
          ...
          return designations.entrySet().stream().map(group -> {
            ...
            if (preferredDesignationCount > 1) {
              return new ChecklistAssertionError().setError(String.format("The concept '%s' has more than one preferred designation in '%s' language", v.getCode(), group.getKey()));
            }
            if (preferredDesignationCount == 0) {
              return new ChecklistAssertionError().setError(String.format("The concept '%s' has no preferred designation in '%s' language", v.getCode(), group.getKey()));
            }
            return null;
          });
