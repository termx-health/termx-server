...
public class JobLogRepository extends BaseRepository {
  ...
  public JobLog load(Long id) {
    SqlBuilder sb = new SqlBuilder(SELECT + " from sys.job_log j where j.sys_status = 'A' and j.id = ?", id);
    return getBean(sb.getSql(), bp, sb.getParams());
  }

  public Long create(JobDefinition definition) {
    SqlBuilder sb = new SqlBuilder("insert into sys.job_log (type, source, status, started) " +
  ...
  public void finish(Long id) {
    SqlBuilder sb = new SqlBuilder("update sys.job_log set status = ?, finished = current_timestamp ", ExecutionStatus.COMPLETED);
    sb.append("where sys_status = 'A' and id = ?", id);
    jdbcTemplate.update(sb.getSql(), sb.getParams());
  }

  public void finish(JobLog jobLog, String status) {
    SqlBuilder sb = new SqlBuilder("update sys.job_log set warnings = ?::jsonb, successes = ?::jsonb, errors = ?::jsonb, status = ?, finished = current_timestamp ",
  ...
  public QueryResult<JobLog> query(JobLogQueryParams params) {
    return query(params, p -> {
      SqlBuilder sb = new SqlBuilder("select count(1) from sys.job_log j where j.sys_status = 'A'");
      sb.append(filter(params));
      return queryForObject(sb.getSql(), Integer.class, sb.getParams());

    }, p -> {
      SqlBuilder sb = new SqlBuilder(SELECT + " from sys.job_log j where j.sys_status = 'A'");
